...
모든 HTTP 요청과 스케줄러 처리 결과가 `logs.txt` 에 한 줄씩 쌓입니다. 경로는 `LOG_FILE` 로 바꿀 수 있습니다.

```
[ISO 시각] [레벨] 이벤트명 키=값 키=값 ...
```

이벤트명을 `job.claimed` 처럼 점으로 구분된 고정 문자열로 두고 가변 정보는 `키=값` 으로 뺐습니다. 문장으로 쓰면 사람이 읽기는 좋아도 `grep job.complete.stale` 같은 집계가 안 됩니다.

요청 로그는 미들웨어가 응답의 `finish` 이벤트에서 남깁니다. 핸들러보다 미들웨어가 먼저 끝나기 때문에, 그 시점이 아니면 상태 코드와 소요 시간을 알 수 없습니다.

아래는 `data/` 의 샘플 데이터로 기동한 뒤 몇 가지 요청을 보낸 실제 기록입니다. `jobId` 는 앞 8자만 남겼습니다.

```text
[2026-09-23T02:30:32.091Z] [LOG] scheduler.bootstrap.start
[2026-09-23T02:30:32.101Z] [LOG] job.recovered jobId=9cbd3820… from=pending to=waiting
[2026-09-23T02:30:32.101Z] [LOG] scheduler.bootstrap.end total=1 failed=0
[2026-09-23T02:30:33.015Z] [LOG] job.claimed jobId=514dc73a… processingTime=8
[2026-09-23T02:30:34.015Z] [LOG] job.claimed jobId=661d305d… processingTime=5
[2026-09-23T02:30:35.014Z] [LOG] job.claimed jobId=9cbd3820… processingTime=12
[2026-09-23T02:30:36.015Z] [LOG] job.claimed jobId=8890cd85… processingTime=3
[2026-09-23T02:30:39.023Z] [LOG] job.completed jobId=8890cd85…
[2026-09-23T02:30:39.026Z] [LOG] job.completed jobId=661d305d…
[2026-09-23T02:30:40.590Z] [LOG] job.created jobId=4ca1f000… processingTime=16 reservationTime=9
[2026-09-23T02:30:40.592Z] [LOG] http.request method=POST url=/jobs status=201 ms=7
[2026-09-23T02:30:40.609Z] [LOG] job.updated jobId=4ca1f000… fields=title
[2026-09-23T02:30:40.609Z] [LOG] http.request method=PATCH url=/jobs/{id} status=200 ms=3
[2026-09-23T02:30:40.617Z] [LOG] http.request method=GET url=/jobs/{id} status=404 ms=1
[2026-09-23T02:30:41.010Z] [LOG] job.claimed jobId=4ca1f000… processingTime=16
[2026-09-23T02:30:41.018Z] [LOG] job.completed jobId=514dc73a…
[2026-09-23T02:30:41.689Z] [LOG] job.canceled jobId=4ca1f000… from=pending
[2026-09-23T02:30:41.689Z] [LOG] http.request method=PATCH url=/jobs/{id}/cancel status=200 ms=7
[2026-09-23T02:30:47.022Z] [LOG] job.completed jobId=9cbd3820…
[2026-09-23T02:30:57.010Z] [WARN] job.complete.stale jobId=4ca1f000… claimed=3 current=4 status=canceled
```

첫 세 줄이 재기동 복구입니다. 선점된 채로 남아 있던 job 을 `waiting` 으로 되돌리고, 그 뒤 틱이 정상적으로 집어 갑니다.

마지막 줄이 선점 소유권 판정입니다. `4ca1f000` 은 `41.010` 에 선점(`version` 3)됐다가 `41.689` 에 취소되어 `version` 이 4 가 됐고, 16 초 뒤 처리를 마친 워커가 `claimed=3 current=4` 를 확인하고 **완료를 커밋하지 않았습니다.** 이 줄이 `job.completed` 였다면 사용자의 취소가 덮여 없어졌을 것입니다.

`WARN` 은 오류가 아니라 **정상 흐름에서 버려진 작업**을 뜻합니다. `job.complete.stale`, `job.recovered.stale`, `job.recover.skipped` 가 여기에 해당합니다.

`logs.txt` 자체는 실행 산출물이라 `.gitignore` 에 있습니다. 앱을 한 번 실행하면 위와 같은 기록이 생깁니다.
...
> 측정 환경: Node v24.12.0 / ext4. 테스트가 `os.tmpdir()` 를 쓰므로 `/tmp` 가 tmpfs 인 환경에서는 쓰기 수치가 더 높게 나옵니다. 테스트도 서비스와 동일하게 `AtomicFileAdapter` 로 DB 를 만들기 때문에 아래 쓰기 수치는 **임시 파일 + fsync + rename 비용이 포함된 값**입니다.

| 시나리오                          | 동시성 | 처리량      | p50     | p95     |
| --------------------------------- | ------ | ----------- | ------- | ------- |
| `GET /jobs` (job 100개)           | 1      | 1,914 req/s | 0.5ms   | 0.7ms   |
| `GET /jobs` (job 100개)           | 10     | 2,358 req/s | 3.9ms   | 4.9ms   |
| `GET /jobs` (job 100개)           | 100    | 2,398 req/s | 18.7ms  | 272.8ms |
| `POST /jobs`                      | 25     | 443 req/s   | 55.6ms  | 61.0ms  |
| `PATCH /jobs/:id` (서로 다른 job) | 50     | 404 req/s   | 120.9ms | 128.5ms |

읽기와 쓰기의 차이가 그대로 드러납니다. `node-json-db` 는 파일을 **최초 1회만 읽어 메모리에 들고** 있으므로 조회는 디스크를 타지 않고, 반대로 모든 쓰기는 **메모리 전체를 직렬화해 파일을 통째로 다시 씁니다**. 즉 읽기는 데이터 크기와 거의 무관하고 쓰기는 전체 job 수에 비례합니다. 동시성을 100 까지 올려도 읽기 처리량이 더 늘지 않는 것도 같은 이유로, 라이브러리 내부의 전역 read-write 락이 접근을 직렬화하기 때문입니다.

쓰기가 읽기보다 다섯 배 이상 느린 데에는 원자적 쓰기의 몫도 있습니다. 같은 부하를 기본 `FileAdapter`(단순 덮어쓰기)로 측정하면 `PATCH` 가 995 req/s 였는데, 임시 파일 + fsync + rename 을 거치면서 404 req/s 로 떨어졌습니다. **처리량의 60% 를 내구성과 맞바꾼 셈입니다.** 쓰기마다 디스크 동기화를 강제하므로 예상된 비용이고, 이 과제에서는 "비정상 종료 시 데이터가 깨지지 않는다" 가 처리량보다 우선한다고 판단했습니다. 쓰기 처리량이 중요해지는 시점이라면 애초에 단일 JSON 파일을 벗어나야 합니다.
...
| 16     | 32   | 272  | 240  | 88.2%  | 8.50        |
| 32     | 32   | 463  | 431  | 93.1%  | 14.47       |
...
### 작업 로직에 따른 상태 전이는 범위에서 제외했습니다

`process()` 는 `processingTime` 만큼 기다리는 `setTimeout` 입니다. 실제 작업 내용이 없으므로 **워커는 job 레코드에 `status` 외에는 아무것도 쓰지 않습니다.**

위의 소유권 판정이 성립하는 것도 이 전제 덕분입니다. 선점 이후의 유일한 쓰기가 선점 자신이기 때문에 "`version` 이 선점 시점 그대로다 = 아무도 손대지 않았다" 가 참이고, 그래서 복구도 값을 되돌릴 필요 없이 `status` 만 되돌리면 됩니다.

실제 작업 로직이 들어가면 전제가 깨집니다. 작업이 중간 결과나 진행 단계를 job 에 기록하면 그 쓰기도 `version` 을 올리므로, 처리 실패 시 `version` 불일치로 판정되어 복구 대상에서 빠집니다. 그 job 은 `pending` 에 남고 재기동 복구에서도 제외되므로 아무도 꺼내주지 않습니다.

이 경우 `version` 하나로는 **누가 썼는지**(워커인지 사용자인지)를 구분할 수 없어 판정 기준 자체를 다시 잡아야 합니다. 워커만 쓰는 필드를 분리하거나, 처리 결과를 job 레코드가 아닌 별도 저장소에 두는 설계가 선행되어야 한다고 봤습니다. 이번 범위에서는 작업을 순수한 대기로 두고, 상태 전이는 스케줄러와 API 만 일으키도록 한정했습니다.
...
### 쓰는 도중에 죽으면 파일 자체가 깨집니다

복구 순서를 아무리 맞춰도, 그 전제는 **디스크에 남은 파일을 읽을 수 있다** 는 것입니다. 그런데 `node-json-db` 의 기본 `FileAdapter` 는 `open(파일, 'w')` 로 기존 파일을 비운 뒤 곧바로 씁니다. 쓰는 도중에 프로세스가 죽으면 JSON 이 중간에서 잘린 채 남고, 다음 기동은 파싱 단계에서 실패합니다. 모든 쓰기가 파일 전체 재작성이라 이 창이 매 쓰기마다 열립니다.

`IAdapter` 가 `readAsync` / `writeAsync` 두 메서드뿐이라 파일 I/O 만 교체할 수 있습니다. `AtomicFileAdapter` 를 만들어 `ConfigWithAdapter` 로 끼웠습니다.

```
임시 파일에 쓰기 → fsync → close → rename
```

`rename` 은 같은 파일시스템 안에서 원자적이라, 어느 시점에 죽어도 **온전한 이전 파일 또는 온전한 새 파일** 둘 중 하나만 남습니다. 임시 파일을 `data/` 안에 두는 것도 이 때문으로, 다른 파일시스템으로 옮기면 내부적으로 복사 후 삭제가 되어 원자성이 깨집니다.

`rename` 전의 `fsync` 가 빠지면 "파일은 안 깨지지만 내용이 비어 있는" 결과가 나올 수 있어서 순서를 지켰습니다. 기본 어댑터의 `syncOnSave` 옵션은 `ConfigWithAdapter` 경로에서는 적용되지 않으므로 어댑터가 직접 합니다.

읽기 쪽에서 파일이 없을 때 `null` 을 돌려주는 것도 같은 인터페이스 계약입니다. 그래야 최초 기동에서 빈 DB 로 초기화되고 파일이 새로 만들어집니다. 반대로 쓰기 실패는 삼키지 않고 그대로 던집니다. 저장에 실패했는데 성공한 것처럼 반환하면, `save()` → `push()` → 서비스로 이어지는 오류 전파가 끊겨서 API 는 200 을 주고 스케줄러는 실패를 감지하지 못합니다.
...
1. **재시도 상한과 격리.** 처리 중 예외가 나면 `waiting` 으로 되돌려 다시 처리되도록 해 뒀습니다(`recoverFailedJob`). 다만 **재시도 횟수를 세지 않기 때문에 상한이 없습니다.** 특정 job 이 항상 실패하면 `선점 → 실패 → waiting 복구 → 재선점` 을 무한히 반복합니다. 실패할 때마다 슬롯은 반납하지만 다음 틱에 곧바로 다시 잡으므로, 결과적으로 동시 실행 슬롯 하나를 계속 소모하면서 `logs.txt` 를 채웁니다.

   상한을 두려면 재시도 횟수를 **선점 시점에** 올려야 합니다. 실패 시점에 올리면 프로세스가 통째로 죽는 실패는 `catch` 가 돌지 않아 집계에서 빠지고, 그러면 기동할 때마다 같은 job 으로 죽는 상황에서 횟수가 영원히 0 에 머뭅니다. 상한을 넘긴 job 은 별도 상태로 격리해 사람이 확인하도록 하고, 재시도 간격도 즉시가 아니라 점점 늘리는 편이 맞다고 봤습니다(아래 3번의 `reservationTime` 이 그 자리입니다).
2. **`reservationTime` 을 실제로 사용.** 현재 값은 저장만 되고 스케줄러가 참조하지 않습니다. 예약 시각이 지난 job 만 선점하도록 하면 1번의 재시도 백오프도 이 필드로 표현할 수 있습니다. 이때 "서버 기동 후 n 초" 라는 상대값은 재기동하면 의미가 사라지므로 절대 시각으로 바꿔야 합니다.
3. **선점 배치화.** 지금은 job 하나를 선점할 때마다 `recovers.json` 과 `jobs.json` 을 각각 통째로 다시 씁니다. N 건을 한 번에 선점하면 2N 번이 2번으로 줄지만, 여러 id 의 mutex 를 동시에 잡아야 해서 획득 순서를 정하지 않으면 교착이 생깁니다.
4. **목록 페이지네이션.** `GET /jobs` 가 전체를 반환합니다.
